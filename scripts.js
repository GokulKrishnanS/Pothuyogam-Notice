document.addEventListener("DOMContentLoaded", ()=>{
	...
	const normalAgenda= "പ്രതിമാസ കരയോഗം നടപടികൾ, വാത്തികുളം ദേവീക്ഷേത്ര സംബന്ധമായ കാര്യങ്ങൾ, ഉത്സവം ഒരു അവലോകനം, മറ്റ് അത്യാവശ്യകാര്യങ്ങൾ";

	mTime =getLocalValues("meetingTime");
	meetingTimeElement.textContent = (mTime !== null && mTime !== "") ? mTime : "പൊതുയോഗ സമയം";

	cDate =getLocalValues("createDate");
	createDateElement.textContent = (cDate !== null && cDate !== "") ? cDate : "തീയതി";

	localAgenda=getLocalValues("localAgenda");
	agendaElement.textContent = (localAgenda !== null && localAgenda !== "") ? localAgenda : normalAgenda;
    ...
    html2canvas(imageDiv, { useCORS: true }).then(async (canvas) => {
        ...
        if (navigator.canShare && navigator.canShare({ files: [] })) {
            try {
                ...
                const file = new File([blob], "div-snapshot.png", { type: "image/png" });
